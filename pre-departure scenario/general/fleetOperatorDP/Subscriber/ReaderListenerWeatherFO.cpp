...
#include "ReaderListenerWeatherFO.h" // Header for weather info listener
...
// Constructor for ReaderListenerWeather, initializes with ReaderCondSync object
ReaderListenerWeather::ReaderListenerWeather(OpenDDS::Model::ReaderCondSync &rcs)
    : rcs_(rcs) {}

// This method is called when new weather data becomes available from the reader
void ReaderListenerWeather::on_data_available(DDS::DataReader_ptr reader)
{
    ...
    // Narrow the generic DataReader to the specific type (weatherInfoDataReader)
    ...
    while (true)
    {
        ...
        if (error == DDS::RETCODE_OK) // If the sample was successfully retrieved
        {
            ...
            {
                // Output the weather information to the console
                ...
                // Open file in append mode and write the weather information to it
