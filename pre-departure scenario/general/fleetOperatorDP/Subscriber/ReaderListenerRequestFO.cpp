...
#include "ReaderListenerRequestFO.h" // Header for booking flight request listener

// Constructor for ReaderListenerRequest, initializes with ReaderCondSync object
...
void ReaderListenerRequest::on_data_available(DDS::DataReader_ptr reader)
{
    ...
    // Narrow the generic DataReader to the specific type (bookingFlightRequestDataReader)
    ...
    while (true)
    {
        ...
        if (error == DDS::RETCODE_OK) // If the sample was successfully retrieved
        {
            if (info.valid_data) // If the sample contains valid data
            {
                ...
                // Append the booking flight request data to a text file
