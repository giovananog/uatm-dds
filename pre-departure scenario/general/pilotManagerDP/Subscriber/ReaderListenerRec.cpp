...
#include <tools/modeling/codegen/model/NullReaderListener.h> // Null reader listener for code generation

#include <model/Sync.h> // Synchronization utilities
#include <ace/Log_Msg.h> // ACE logging utilities

#include "ReaderListenerRec.h" // The header for the flight change record listener
#include <dds/DCPS/WaitSet.h> // DDS WaitSet for managing events

// Constructor for the ReaderListenerRec class, which initializes with a synchronization object
ReaderListenerRec::ReaderListenerRec(OpenDDS::Model::ReaderCondSync &rcs)
    : rcs_(rcs) {} // Initialize the ReaderCondSync object for synchronization
...
void ReaderListenerRec::on_data_available(DDS::DataReader_ptr reader)
{
  ...
  // Narrow the generic DataReader to a specific type: flightChangeRecDataReader
  ...
  while (true)
  {
    ...
    if (error == DDS::RETCODE_OK) // If the sample was successfully retrieved
    {
      ...
      {
        // Output the flight change record data to the console
