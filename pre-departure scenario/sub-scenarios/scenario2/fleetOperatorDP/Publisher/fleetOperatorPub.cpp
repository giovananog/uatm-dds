...
int ACE_TMAIN(int argc, ACE_TCHAR **argv)
{
    try
    {
        ...
        while (true)
        {
            ...
            {
                if (checkWeatherConditions(weatherFile, "Skyport-1", weatherID)) 
                {
                    if (checkRouteAvailability(routeFile, "Skyport-1", "Skyport-2", routeID))
                    {
                        std::string flightFile = "fleetOperatorDP/data/requests.txt"; 

                        if (findAndAssignFlight(flightFile, evtolID, pilotID, flightID, weatherID, routeID))
                        {
                    ...
                    {
                        std::cout << "Nenhuma rota disponível ou condições de tráfego desfavoráveis." << std::endl;
                    }
                }
                else
                {
                    std::cout << "Condições meteorológicas desfavoráveis." << std::endl;
                }
            }
            else
            {
